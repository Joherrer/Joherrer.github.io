...
(function () {
  ...
  function updateScrollTopState() {
    ...
    const shouldBeActive =
      window.scrollY > 100 &&
      !selectBody.classList.contains("mobile-nav-active");
  ...
  function clearScaledProjectCards() {
    document
      .querySelectorAll(".portfolio .portfolio-card.scaled")
      .forEach((card) => {
  ...
  function initMobileNavDelay() {
    const elements = document.querySelectorAll(
      ".header .header-logo, .navbar-nav .nav-link",
    );
    ...
    elements.forEach((el) => {
      el.addEventListener("click", function (e) {
        ...
        if (this.classList.contains("nav-link") && href && href !== "#") {
          e.preventDefault();
          document
            .querySelectorAll(".navbar-nav .nav-link.active")
            .forEach((link) => {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          showAlert(
            data.errors ? data.errors[0].message : "Error occurred",
            "danger",
          );
        }
      } catch {
